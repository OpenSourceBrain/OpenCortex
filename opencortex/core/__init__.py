...
def include_neuroml2_cell(nml_doc, cell_nml2_path, cell_id, channels_also=True):
    ...
    return oc_build._include_neuroml2_cell(
        nml_doc, cell_nml2_path, cell_id, channels_also=channels_also
    )
...
def include_neuroml2_cell_and_channels(nml_doc, cell_nml2_path, cell_id):
    ...
    return oc_build._include_neuroml2_cell(
        nml_doc, cell_nml2_path, cell_id, channels_also=True
    )
...
def include_opencortex_cell(nml_doc, reference):
    ...
    return oc_build._add_cell_and_channels(
        nml_doc, reference, cell_id, use_prototypes=True
    )
...
def add_exp_one_syn(nml_doc, id, gbase, erev, tau_decay):
    ...
    """
    syn0 = neuroml.ExpOneSynapse(id=id, gbase=gbase, erev=erev, tau_decay=tau_decay)
...
def add_exp_two_syn(nml_doc, id, gbase, erev, tau_rise, tau_decay):
    ...
    syn0 = neuroml.ExpTwoSynapse(
        id=id, gbase=gbase, erev=erev, tau_rise=tau_rise, tau_decay=tau_decay
    )
...
def add_transient_poisson_firing_synapse(
    ...
    return oc_build._add_transient_poisson_firing_synapse(
        nml_doc, id, average_rate, delay, duration, synapse_id
    )
...
def add_voltage_clamp_triple(
    ...
    return oc_build._add_voltage_clamp_triple(
        nml_doc,
        id,
        delay,
        duration,
...
def add_single_cell_population(net, pop_id, cell_id, x=0, y=0, z=0, color=None):
    ...
    pop = neuroml.Population(
        id=pop_id, component=cell_id, type="populationList", size=1
    )
...
def add_population_in_rectangular_region(
    net,
    pop_id,
    cell_id,
    size,
    ...
    return oc_build._add_population_in_rectangular_region(
        net,
        pop_id,
        cell_id,
        size,
...
def add_probabilistic_projection(
    net,
    prefix,
    ...
    proj = neuroml.Projection(
        id="%s_%s_%s" % (prefix, presynaptic_population.id, postsynaptic_population.id),
...
def add_targeted_projection(
    ...
    if presynaptic_population.component in oc_build.cell_ids_vs_nml_docs:
        ...
        ].get_by_id(presynaptic_population.component)
    else:
    ...
    if postsynaptic_population.component in oc_build.cell_ids_vs_nml_docs:
        ...
        ].get_by_id(postsynaptic_population.component)
    else:
        post_cell = "Undetermined"

    if pre_segment_group:
        pre_segs = oc_build.extract_seg_ids(pre_cell, [pre_segment_group], "segGroups")
    ...
    if post_segment_group:
        post_segs = oc_build.extract_seg_ids(
            post_cell, [post_segment_group], "segGroups"
        )
    ...
    for synapse in synapse_list:
        proj_id = (
            "%s_%s_%s" % (prefix, presynaptic_population.id, postsynaptic_population.id)
            if len(synapse_list) == 1
            else "%s_%s_%s_%s"
            % (prefix, presynaptic_population.id, postsynaptic_population.id, synapse)
        )
...
def add_targeted_electrical_projection(
    nml_doc,
    net,
    prefix,
    presynaptic_population,
    ...
    oc_build._add_elect_projection(
        net,
        projections,
        presynaptic_population,
        postsynaptic_population,
...
def add_inputs_to_population(
    ...
    if len(segment_ids) != 1 and len(segment_ids) != number_per_cell:
        error = (
            "Error! Attribute segment_ids in method opencortex.build.%s()"
            % sys._getframe().f_code.co_name
    ...
    if len(fraction_alongs) != 1 and len(fraction_alongs) != number_per_cell:
        error = (
            "Error! Attribute fraction_alongs in method opencortex.build.%s()"
            % sys._getframe().f_code.co_name
    ...
    input_list = neuroml.InputList(
        id=id, component=input_comp_id, populations=population.id
    )
    count = 0
...
def add_targeted_inputs_to_population(
    net,
    id,
    population,
    input_comp_id,
    ...
    target_cell = oc_build.cell_ids_vs_nml_docs[population.component].get_by_id(
        population.component
    )

    target_segs = oc_build.extract_seg_ids(target_cell, [segment_group], "segGroups")
...
def save_network(
    ...
    oc_build._finalise_copy_to_dir_for_model(
        nml_doc, target_dir, use_subfolder=use_subfolder
    )
    ...
    opencortex.print_comment_v(
        "Saved NeuroML with id: %s to file: %s" % (nml_doc.id, abs_path)
    )
...
def generate_lems_simulation(
    ...
    quantities_saved, lems_sim = pyneuroml.lems.generate_lems_file_for_neuroml(
        "Sim_%s" % network.id,
        nml_file_name,
        network.id,
        duration,
