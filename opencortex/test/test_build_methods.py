...
class TestNetMorphMethods(unittest.TestCase):
    ...
    def test_extract_seg_ids(self):
        ...
        test_return1 = oc_build.extract_seg_ids(
            cell_object,
        ...
        self.assertEqual(
            set(["basal_obl_dends", "most_prox_bas_dend"]), set(test_return1.keys())
        )

        self.assertEqual(
            set(test_return1["basal_obl_dends"]),
            set(
                [
                    16,
                    17,
        ...
        self.assertEqual(
            set(test_return1["most_prox_bas_dend"]),
            set([16, 17, 14, 15, 12, 13, 10, 11, 8, 9, 6, 7, 4, 5, 2, 3]),
        )

        test_return2 = oc_build.extract_seg_ids(
            cell_object,
    ...
    def test_make_target_dict(self):
        ...
        makeDict1 = oc_build.make_target_dict(
        ...
        self.assertEqual(
            set(makeDict1["Seg1_comp_28"].keys()), set(["SegList", "LengthDist"])
        )

        self.assertEqual(
            len(makeDict1["Seg1_comp_68"]["SegList"]),
            len(makeDict1["Seg1_comp_68"]["LengthDist"]),
        )

        makeDict2 = oc_build.make_target_dict(
            cell_object=cell_object,
            target_segs={
                "basal_obl_dends": [
                    16,
        ...
        self.assertEqual(
            len(makeDict2["basal_obl_dends"]["SegList"]),
            len(makeDict2["basal_obl_dends"]["LengthDist"]),
        )

        self.assertEqual(
            set(makeDict2["most_prox_bas_dend"]["SegList"]),
            set([16, 17, 14, 15, 12, 13, 10, 11, 8, 9, 6, 7, 4, 5, 2, 3]),
        )

    #######################################################################################################################
    def test_get_target_segments(self):
        target_segs1, fractions_list1 = oc_build.get_target_segments(
            seg_specifications={
                "Seg1_comp_28": {"SegList": [55], "LengthDist": [25.00030750752478]},
                "Seg1_comp_68": {"SegList": [135], "LengthDist": [24.99966139974902]},
            },
            subset_dict={"Seg1_comp_x": 1, "Seg1_comp_y": 1},
        )
        ...
        target_segs1, fractions_list1 = oc_build.get_target_segments(
            ...
            subset_dict={"Seg1_comp_28": 1, "Seg1_comp_68": 1},
        )
        ...
        target_segs, fractions_list = oc_build.get_target_segments(
            seg_specifications={
                "basal_obl_dends": {
                    "SegList": [
                        16,
            ...
            subset_dict={"basal_obl_dends": 100, "Seg1_comp_68": 50},
        )
...
class TestNetConnectionMethods(unittest.TestCase):
    def test_add_targeted_projection_by_dicts(self):
        ...
        presynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrRS", type="populationList", size=1
        )
        postsynaptic_population = neuroml.Population(
            id="Pop1", component="L23PyrFRB", type="populationList", size=1
        )
        ...
        self.assertEqual(
            len(proj_array[0].connection_wds),
            len(network.projections[0].connection_wds),
        )
        ...
        for conn_ind in range(0, 50):
            pre_cell_AMPA_strings.append(
                proj_array[0].connection_wds[conn_ind].pre_cell_id
            )

            post_cell_AMPA_strings.append(
                proj_array[0].connection_wds[conn_ind].post_cell_id
            )

            pre_cell_NMDA_strings.append(
                proj_array[1].connection_wds[conn_ind].pre_cell_id
            )

            post_cell_NMDA_strings.append(
                proj_array[1].connection_wds[conn_ind].post_cell_id
            )
        ...
        self.assertEqual(
            len(set(post_cell_AMPA_strings)), len(set(post_cell_NMDA_strings))
        )

        self.assertEqual(
            len(proj_array[0].connection_wds), len(proj_array[1].connection_wds)
        )
        ...
        network = neuroml.Network(id="Net0")
        presynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrRS", type="populationList", size=50
        )
        postsynaptic_population = neuroml.Population(
            id="Pop1", component="L23PyrFRB", type="populationList", size=2
        )
        ...
        for conn_ind in range(0, 100):
            ...
            self.assertEqual(
                proj_array[0].connection_wds[conn_ind].post_segment_id,
                proj_array[1].connection_wds[conn_ind].post_segment_id,
            )

            self.assertEqual(
                proj_array[0].connection_wds[conn_ind].pre_segment_id,
                proj_array[1].connection_wds[conn_ind].pre_segment_id,
            )

            self.assertEqual(
                proj_array[0].connection_wds[conn_ind].pre_fraction_along,
                proj_array[1].connection_wds[conn_ind].pre_fraction_along,
            )

            self.assertEqual(
                proj_array[0].connection_wds[conn_ind].post_fraction_along,
                proj_array[1].connection_wds[conn_ind].post_fraction_along,
            )

            self.assertNotEqual(
                proj_array[0].connection_wds[conn_ind].delay,
                proj_array[1].connection_wds[conn_ind].delay,
            )

            self.assertNotEqual(
                proj_array[0].connection_wds[conn_ind].weight,
        ...
        network = neuroml.Network(id="Net0")
        presynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrRS", type="populationList", size=3
        )
        postsynaptic_population = neuroml.Population(
            id="Pop1", component="L23PyrFRB", type="populationList", size=50
        )
        ...
        for conn_ind in range(0, 100):
            self.assertTrue(
                AMPA_proj[conn_ind].pre_segment_id
                in parsed_pre_target_dict["distal_axon"]["SegList"]
            )

            self.assertTrue(
                AMPA_proj[conn_ind].pre_segment_id == NMDA_proj[conn_ind].pre_segment_id
            )

            self.assertTrue(
                AMPA_proj[conn_ind].post_segment_id
                in parsed_target_dict["basal_obl_dends"]["SegList"]
            )

            self.assertTrue(
                AMPA_proj[conn_ind].post_segment_id
                == NMDA_proj[conn_ind].post_segment_id
            )
        ...
        for conn_ind in range(0, 100):
            ...
            check_distal = (
                AMPA_proj[conn_ind].post_segment_id
                in parsed_target_dict["distal_axon"]["SegList"]
            )
    ...
    def test_add_chem_spatial_projection(self):
        ...
        self.assertTrue(
            len(proj_array[0].connection_wds) == len(set(post_cell_AMPA_strings))
        )
    ...
    def test_add_elect_projection(self):
        ...
        postsynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrFRB", type="populationList", size=1
        )
        ...
        self.assertEqual(
            len(proj_array[1].electrical_connection_instance_ws),
            len(proj_array[0].electrical_connection_instance_ws),
        )

        ######## Test 2 convergent
        network = neuroml.Network(id="Net0")
        presynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrRS", type="populationList", size=2
        )
        postsynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrFRB", type="populationList", size=1
        )
        ...
        self.assertEqual(
            len(proj_array[0].electrical_connection_instance_ws),
            len(network.electrical_projections[0].electrical_connection_instance_ws),
        )
        ...
        self.assertTrue(
            len(proj_array[0].electrical_connection_instance_ws) == 2
            or len(proj_array[0].electrical_connection_instance_ws) == 3
        )
        ...
        self.assertEqual(
            network.electrical_projections[0].presynaptic_population,
            proj_array[0].presynaptic_population,
        )

        self.assertEqual(
            network.electrical_projections[1].presynaptic_population,
            proj_array[1].presynaptic_population,
        )

        self.assertEqual(
            network.electrical_projections[0].postsynaptic_population,
            proj_array[0].postsynaptic_population,
        )

        self.assertEqual(
            network.electrical_projections[1].postsynaptic_population,
            proj_array[1].postsynaptic_population,
        )
        ...
        self.assertTrue(
            len(proj_array[0].electrical_connection_instance_ws) == 0
            or len(proj_array[0].electrical_connection_instance_ws) == 1
        )
        ...
        if len(proj_array[0].electrical_connection_instance_ws) == 1:
            ...
            self.assertEqual(
                len(proj_array[0].electrical_connection_instance_ws),
                len(
                    network.electrical_projections[0].electrical_connection_instance_ws
                ),
            )
        ...
        if len(proj_array[0].electrical_connection_instance_ws) == 0:
            ...
            for elect_conn_ind in range(
                0,
                len(
                    network.electrical_projections[0].electrical_connection_instance_ws
                ),
            ):
                elect_connection_comp1 = proj_array[
                    0
                ].electrical_connection_instance_ws[elect_conn_ind]

                elect_connection_comp2 = proj_array[
                    1
                ].electrical_connection_instance_ws[elect_conn_ind]

                self.assertTrue(
                    elect_connection_comp1.pre_cell == elect_connection_comp2.pre_cell
                )

                self.assertTrue(
                    elect_connection_comp1.post_cell == elect_connection_comp2.post_cell
                )
        ...
        for elect_conn_ind in range(
            0, len(network.electrical_projections[0].electrical_connection_instance_ws)
        ):
            elect_connection_comp1 = proj_array[0].electrical_connection_instance_ws[
                elect_conn_ind
            ]

            elect_connection_comp2 = proj_array[1].electrical_connection_instance_ws[
                elect_conn_ind
            ]
    ...
    def test_add_elect_spatial_projection(self):
        ...
        self.assertEqual(
            len(network.electrical_projections[0].electrical_connection_instance_ws),
            len(network.electrical_projections[1].electrical_connection_instance_ws),
        )
        ...
        for conn_ind in range(
            0, len(network.electrical_projections[0].electrical_connection_instance_ws)
        ):
            self.assertEqual(
                network.electrical_projections[0]
                .electrical_connection_instance_ws[conn_ind]
                .synapse,
                "Elect_1",
            )

            self.assertEqual(
                network.electrical_projections[1]
                .electrical_connection_instance_ws[conn_ind]
                .synapse,
                "Elect_2",
            )

            pre_cell_Elect1_strings.append(
                network.electrical_projections[0]
                .electrical_connection_instance_ws[conn_ind]
                .pre_cell
            )

            post_cell_Elect1_strings.append(
                network.electrical_projections[0]
                .electrical_connection_instance_ws[conn_ind]
                .post_cell
            )

            pre_cell_Elect2_strings.append(
                network.electrical_projections[1]
                .electrical_connection_instance_ws[conn_ind]
                .pre_cell
            )

            post_cell_Elect2_strings.append(
                network.electrical_projections[1]
                .electrical_connection_instance_ws[conn_ind]
                .post_cell
            )
        ...
        self.assertTrue(
            len(network.electrical_projections[0].electrical_connection_instance_ws)
            == len(set(pre_cell_Elect1_strings))
        )

        self.assertEqual(
            len(set(pre_cell_Elect1_strings)), len(set(pre_cell_Elect2_strings))
        )

        self.assertEqual(
            len(set(post_cell_Elect1_strings)), len(set(post_cell_Elect2_strings))
        )
        ...
        self.assertTrue(
            len(network.electrical_projections[0].electrical_connection_instance_ws)
            == len(set(post_cell_Elect1_strings))
        )
    ...
    def test_add_probabilistic_projection_list(self):
        ...
        postsynaptic_population = neuroml.Population(
            id="Pop0", component="L23PyrFRB", type="populationList", size=50
        )
        ...
        self.assertEqual(
            len(network.projections[0].connection_wds),
            len(network.projections[1].connection_wds),
        )

        for proj_ind in range(0, len(network.projections)):
            self.assertTrue(
                "Syn_1" in network.projections[proj_ind].id
                or "Syn_2" in network.projections[proj_ind].id
            )

            self.assertTrue(
                network.projections[proj_ind].synapse == "Syn_1"
                or network.projections[proj_ind].synapse == "Syn_2"
            )
        ...
        for proj_ind in range(0, len(network.projections)):
            ...
            for conn_ind in range(0, len(network.projections[proj_ind].connection_wds)):
                ...
                if "Syn_1" in network.projections[proj_ind].id:
                    self.assertTrue(
                        connection.weight != 2
                        and connection.weight > 0
                        and connection.weight < 5
                    )
        ...
        for proj_ind in range(0, len(network.projections)):
            ...
            for conn_ind in range(0, len(network.projections[proj_ind].connection_wds)):
                ...
                if "Syn_1" in network.projections[proj_ind].id:
                    ...
                    self.assertTrue(
                        connection.get_delay_in_ms() != 5
                        and connection.get_delay_in_ms() > 3
                        and connection.get_delay_in_ms() < 7
                    )
                ...
                if "Syn_2" in network.projections[proj_ind].id:
                    ...
                    self.assertTrue(
                        connection.get_delay_in_ms() != 1
                        and connection.get_delay_in_ms() > 0
                        and connection.get_delay_in_ms() < 2
                    )
