...
class TestUtilsMethods(unittest.TestCase):
    def test_read_connectivity(self):
        proj_info = oc_utils.read_connectivity(
            pre_pop="CG3D_L23PyrRS",
            post_pop="CG3D_L23PyrFRB",
            path_to_txt_file="ConnListTest",
        )
        ...
        for proj_ind in range(0, len(proj_info)):
            self.assertEqual(
                len(proj_info[proj_ind].keys()), len(proj_info[proj_ind].values())
            )
            ...
            self.assertTrue(
                "NumPerPostCell" in proj_info[proj_ind].keys()
                or "NumPerPreCell" in proj_info[proj_ind].keys()
            )
    ...
    def test_add_populations_in_rectangular_layers(self):
        ...
        for pop_index in range(0, len(network.populations)):
            ...
            for cell_loc in range(0, len(pop.instances)):
                ...
                self.assertTrue(
                    boundaries[popDict[pop.id][1]][0] >= location.y
                    and location.y >= boundaries[popDict[pop.id][1]][1]
                )
                ...
                for cell_loc_inner in range(0, len(pop.instances)):
                    if cell_loc != cell_loc_inner:
                        ...
                        d = oc_build.distance(
                            ...
                            [inner_location.x, inner_location.y, inner_location.z],
                        )

                        self.assertTrue(
                            d
                            < (
                                cell_diameters[pop.component]
                                + cell_diameters[pop.component]
                            )
                            / 2
                        )
                        ...
                        for cell_loc_inner in range(0, len(pop_inner.instances)):
                            if cell_loc != cell_loc_inner:
                                ...
                                self.assertTrue(
                                    d
                                    < (
                                        cell_diameters[pop_inner.component]
                                        + cell_diameters[pop_inner.component]
                                    )
                                    / 2
                                )
    ...
    def test_add_populations_in_cylindrical_layers(self):
        ...
        for pop_index in range(0, len(network.populations)):
            ...
            for cell_loc in range(0, len(pop.instances)):
                ...
                self.assertTrue(
                    oc_build.distance([location.x, location.z], [0, 0]) <= 250
                )
        ...
        pop_params = oc_utils.add_populations_in_cylindrical_layers(
            net=network, boundaryDict=boundaries, popDict=popDict, radiusOfCylinder=250
        )
        ...
        for pop_index in range(0, len(network.populations)):
            ...
            for cell_loc in range(0, len(pop.instances)):
                ...
                check_x = (
                    abs(location.x - stored_cell_positions[cell_loc][0]) < 0.00000001
                )

                check_y = (
                    abs(location.y - stored_cell_positions[cell_loc][1]) < 0.00000001
                )

                check_z = (
                    abs(location.z - stored_cell_positions[cell_loc][2]) < 0.00000001
                )
        ...
        for pop_index in range(0, len(network.populations)):
            ...
            for cell_loc in range(0, len(pop.instances)):
                ...
                for side_index in range(0, len(xy_sides)):
                    if (
                        abs(
                            vertex_array[side_index][1]
                            - vertex_array[side_index - 1][1]
                        )
                        > 0.0000001
                    ):
                        if (
                            location.z < vertex_array[side_index][1]
                            and location.z > vertex_array[side_index - 1][1]
                        ):
                            if (
                                xy_sides[side_index][0] != None
                                and xy_sides[side_index][1] == None
                            ):
                            ...
                            if (
                                ...
                                if (
                                    location.x
                                    <= (location.z - xy_sides[side_index][1])
                                    / xy_sides[side_index][0]
                                ):
                                    count_intersections += 1

                        if (
                            location.z < vertex_array[side_index - 1][1]
                            and location.z > vertex_array[side_index][1]
                        ):
                            if (
                                xy_sides[side_index][0] != None
                                and xy_sides[side_index][1] == None
    ...
    def test_check_cached_dicts(self):
        ...
        PostSegLengthDict, cached_target_dict = oc_utils.check_cached_dicts(
            cell_component="Test",
        ...
        self.assertTrue(
            cached_target_dict["Test"]["TargetDict"]["basal_obl_dends"]["SegList"]
            != None
        )

        self.assertTrue(
            cached_target_dict["Test"]["TargetDict"]["basal_obl_dends"]["LengthDist"]
            != None
        )
        ...
        self.assertTrue(
            cached_target_dict["Test"]["TargetDict"]["distal_axon"]["SegList"] != None
        )

        self.assertTrue(
            cached_target_dict["Test"]["TargetDict"]["distal_axon"]["LengthDist"]
            != None
        )
    ...
    def test_build_connectivity(self):
        ...
        all_synapse_components, proj_array, cached_segment_dicts = (
            oc_utils.build_connectivity(
                ...
                pre_segment_group_info=[
                    {"PreSegGroup": "distal_axon", "ProjType": "Chem"}
                ],
                synaptic_scaling_params=[
                    {
                        "weight": 2.0,
                        "synComp": "AMPA",
                        "synEndsWith": [],
        ...
        for pre_pop_id in popDict.keys():
            for post_pop_id in popDict.keys():
                proj_summary = oc_utils.read_connectivity(
                    pre_pop=pre_pop_id,
                    post_pop=post_pop_id,
                    path_to_txt_file="ConnListTest",
                )
                    ...
                    for syn_ind in range(0, len(synapse_list)):
                        if projInfo["Type"] == "Chem":
                            for net_proj in range(0, len(network.projections)):
                                ...
                                if (
                                    proj.presynaptic_population == pre_pop_id
                                    and proj.postsynaptic_population == post_pop_id
                                    and proj.synapse == synapse_list[syn_ind]
                                ):
                                    ...
                                    self.assertTrue(
                                        len(proj.connection_wds)
                                        == float(num_per_post_cell)
                                        * popDict[post_pop_id][0]
                                    )

                                    for conn_index in range(
                                        0, len(proj.connection_wds)
                                    ):
                                        connection = proj.connection_wds[conn_index]

                                        self.assertTrue(
                                            connection.pre_segment_id in pre_segments
                                        )

                                        self.assertTrue(
                                            connection.post_segment_id in post_segments
                                        )

                                        delay_value = float(
                                            connection.delay.split(" ")[0]
                                        )
                        ...
                        if projInfo["Type"] == "Elect":
                            for net_proj in range(
                                0, len(network.electrical_projections)
                            ):
                                proj = network.electrical_projections[net_proj]

                                check_synapse = (
                                    proj.electrical_connection_instance_ws[0].synapse
                                    == synapse_list[syn_ind]
                                )
                                ...
                                if (
                                    ...
                                    for conn_index in range(
                                        0, len(proj.electrical_connection_instance_ws)
                                    ):
                                        connection = (
                                            proj.electrical_connection_instance_ws[
                                                conn_index
                                            ]
                                        )

                                        self.assertTrue(
                                            connection.pre_segment in pre_segments
                                        )

                                        self.assertTrue(
                                            connection.post_segment in post_segments
                                        )
    ...
    def test_probability_based_connectivity(self):
        ...
        prob_matrix = [[0.0, 0.5], [0.9, 0.0]]

        syn_matrix = [[None, ["Syn1", "Syn2"]], [["Syn1", "Syn2"], None]]

        pop_tags = ["L23PyrRS", "L23PyrFRB"]

        w_matrix = [[None, 2.0], [1.0, None]]

        d_matrix = [[0.05, 0.05], [0.05, 0.05]]

        proj_array = oc_utils.build_probability_based_connectivity(
            net=network,
        ...
        for proj_ind in range(0, len(network.projections)):
            ...
            for row_index in range(0, len(syn_matrix)):
                for col_index in range(0, len(syn_matrix[row_index])):
                    if isinstance(syn_matrix[row_index][col_index], list):
                        for syn_component in range(
                            0, len(syn_matrix[row_index][col_index])
                        ):
                            check_component = (
                                syn_matrix[row_index][col_index][syn_component]
                                == proj.synapse
                            )
            ...
            for conn_index in range(
                0, len(network.projections[proj_ind].connection_wds)
            ):
                connection = network.projections[proj_ind].connection_wds[conn_index]

                self.assertTrue(
                    connection.weight == w_matrix[post_pop_index][pre_pop_index]
                )

                self.assertTrue(
                    str(d_matrix[post_pop_index][pre_pop_index]) in connection.delay
                )
    ...
    def test_build_inputs(self):
        ...
        CG3D_L23PyrRS_target_segs = oc_build.extract_seg_ids(
            cell_object,
        ...
        CG3D_L23PyrFRB_target_segs = oc_build.extract_seg_ids(
            cell_object,
            target_compartment_array=["dendrite_group"],
            targeting_mode="segGroups",
        )
        ...
        input_params = {
            ...
            "CG3D_L23PyrFRB": [
                ...
                },
                {
                    ...
                    "TargetRegions": [
                        {"XVector": [0, 50], "YVector": [0, -500], "ZVector": [0, 50]}
                    ],
                    "TargetDict": {"dendrite_group": 100},
                },
            ],
        ...
        self.assertTrue(
            isinstance(input_list_array_final, list)
            and len(input_list_array_final) >= 0
        )
        ...
        for pulse_index in range(0, len(nml_doc.pulse_generators)):
            ...
            if ("DepCurr_L23FRB_spatial" in pulse_group.id) and (
                "Pulse0" in pulse_group.id
            ):
        ...
        for poisson_index in range(0, len(nml_doc.transient_poisson_firing_synapses)):
            poisson_synapse_case = nml_doc.transient_poisson_firing_synapses[
                poisson_index
            ]
        ...
        for proj_index in range(0, len(network.projections)):
            ...
            if (
                (proj.presynaptic_population in CG3D_PointNeurons_spike_source_pops)
                and (proj.postsynaptic_population == "CG3D_PointNeurons")
                ...
                for conn_index in range(0, len(proj.connection_wds)):
                    ...
                    self.assertEqual(
                        "../CG3D_PointNeurons/0/SingleCompartment", conn.post_cell_id
                    )
        ...
        for input_list_index in range(0, len(network.input_lists)):
            ...
            if (
                "BackgroundL23RS" in input_list_instance.id
                and "BackgroundL23RS" in input_list_instance.component
            ):
                CG3D_L23PyrRS_persistent_synapse_input_lists.append(
                    input_list_instance.id
                )

                for input_index in range(0, len(input_list_instance.input)):
                    input_case = input_list_instance.input[input_index]

                    CG3D_L23PyrRS_persistent_synapse_input_list_instances.append(
                        input_case.id
                    )

                    self.assertTrue(
                        int(input_case.segment_id) in CG3D_L23PyrRS_target_segs
                    )

                    self.assertTrue(
                        float(input_case.fraction_along) >= 0
                        and float(input_case.fraction_along) <= 1
                    )

            if (
                "EctopicStimL23FRB" in input_list_instance.id
                and "EctopicStimL23FRB" in input_list_instance.component
            ):
                CG3D_L23PyrFRB_transient_synapse_input_lists.append(
                    input_list_instance.id
                )
                ...
                for input_index in range(0, len(input_list_instance.input)):
                    ...
                    CG3D_L23PyrFRB_transient_synapse_input_list_instances.append(
                        input_case.id
                    )
            ...
            if "DepCurr_L23FRB_spatial" in input_list_instance.id and (
                "DepCurr_L23FRB_spatial" in input_list_instance.component
                and "Pulse0" in input_list_instance.component
            ):
                ...
                for input_index in range(0, len(input_list_instance.input)):
                    ...
                    G3D_L23PyrFRB_spatial_pulse0_input_list_instances.append(
                        input_case.id
                    )

                    self.assertTrue(
                        int(input_case.segment_id) in CG3D_L23PyrFRB_target_segs
                    )
            ...
            if "DepCurr_L23FRB_spatial" in input_list_instance.id and (
                "DepCurr_L23FRB_spatial" in input_list_instance.component
                and "Pulse1" in input_list_instance.component
            ):
                ...
                for input_index in range(0, len(input_list_instance.input)):
                    ...
                    G3D_L23PyrFRB_spatial_pulse1_input_list_instances.append(
                        input_case.id
                    )
            ...
            if (
                "DepCurr_L23RS" in input_list_instance.id
                and "DepCurr_L23RS" in input_list_instance.component
            ):
            ...
            if (
                "DepCurr_CG3D_PointNeurons" in input_list_instance.id
                and "DepCurr_CG3D_PointNeurons" in input_list_instance.component
            ):
        ...
        self.assertEqual(
            len(set(CG3D_L23PyrRS_pulses)), len(set(CG3D_L23PyrRS_pulse_input_lists))
        )
        ...
        self.assertEqual(
            len(set(G3D_L23PyrFRB_spatial_pulse0_input_list_instances)), 1000
        )

        self.assertEqual(
            len(set(G3D_L23PyrFRB_spatial_pulse1_input_list_instances)), 1000
        )
        ...
        self.assertEqual(
            len(set(CG3D_L23PyrRS_persistent_synapse_input_list_instances)), 200
        )
        ...
        self.assertEqual(
            len(set(CG3D_L23PyrFRB_transient_synapse_input_list_instances)), 5
        )

        self.assertTrue(len(set(CG3D_PointNeurons_spike_sources)), 1)

        self.assertTrue(
            len(set(CG3D_PointNeurons_spike_source_pops)),
            len(set(CG3D_PointNeurons_spike_sources)),
        )

        self.assertTrue(
            len(set(CG3D_PointNeurons_spike_source_projections)),
            len(set(CG3D_PointNeurons_spike_sources)),
        )
