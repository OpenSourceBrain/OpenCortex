...
def add_populations_in_rectangular_layers(
    net,
    boundaryDict,
    popDict,
    x_vector,
    ...
    for cell_pop in popDict.keys():
        ...
        if size > 0:
            ...
            if storeSoma:
                pop, cellPositions = oc_build._add_population_in_rectangular_region(
                    net,
                    cell_pop,
                    cell_model,
                    size,
                ...
                pop = oc_build._add_population_in_rectangular_region(
                    net,
                    cell_pop,
                    cell_model,
                    size,
    ...
    opencortex.print_comment_v(
        "This is a final list of cell population ids: %s" % return_pops.keys()
    )
...
def add_populations_in_cylindrical_layers(
    net,
    boundaryDict,
    popDict,
    radiusOfCylinder,
    ...
    if numOfSides != None:
        if numOfSides >= 3:
            ...
            angle_array = np.linspace(
                0, 2 * math.pi * (1 - (1.0 / numOfSides)), numOfSides
            )

            opencortex.print_comment_v(
                "Generated the angles of the regular polygon with %d sides: %s"
                % (numOfSides, angle_array)
            )
            ...
            opencortex.print_comment_v(
                "Generated the vertices of the regular polygon with %d sides: %s"
                % (numOfSides, vertex_array)
            )
            ...
            opencortex.print_comment_v(
                "Error! Method opencortex.build.%s() called with numOfSides set to %d but regular polygon must contain at least 3 vertices."
    ...
    for cell_pop in popDict.keys():
        ...
        if size > 0:
            ...
            if storeSoma:
                pop, cellPositions = oc_build.add_population_in_cylindrical_region(
                    net=net,
                    pop_id=cell_pop,
                    cell_id=cell_model,
                    size=size,
                ...
                pop = oc_build.add_population_in_cylindrical_region(
                    net=net,
                    pop_id=cell_pop,
                    cell_id=cell_model,
                    size=size,
...
def build_projection(
    ...
    for synapse_id in synapse_list:
        if proj_type == "Elect":
            proj = neuroml.ElectricalProjection(
                id="Proj%dsyn%d_%s_%s"
                % (
                    proj_counter,
                    syn_counter,
                    presynaptic_population.id,
                    postsynaptic_population.id,
                ),
    ...
    if distance_dependent_rule == None:
        if proj_type == "Chem":
            proj_array = oc_build.add_targeted_projection_by_dicts(
                net,
                proj_array,
                presynaptic_population,
                postsynaptic_population,
        ...
        if proj_type == "Elect":
            proj_array = oc_build._add_elect_projection(
                net,
                proj_array,
                presynaptic_population,
                postsynaptic_population,
        ...
        if proj_type == "Chem":
            proj_array = oc_build.add_chem_spatial_projection(
                net,
                proj_array,
                presynaptic_population,
                postsynaptic_population,
        ...
        if proj_type == "Elect":
            proj_array = oc_build.add_elect_spatial_projection(
                net,
                proj_array,
                presynaptic_population,
                postsynaptic_population,
    ...
    for prePop in pop_objects.keys():
        ...
        for postPop in pop_objects.keys():
            ...
            if preCellObject["PopObj"].size != 0 and postCellObject["PopObj"].size != 0:
                proj_summary = read_connectivity(
                    prePop,
                    postPop,
                    full_path_to_conn_summary,
                    ...
                    for proj_ind in range(0, len(proj_summary)):
                        ...
                        if synaptic_scaling_params != None:
                            weights = parse_weights(
                                synaptic_scaling_params, postPop, synapseList
                            )
                        ...
                        if synaptic_delay_params != None:
                            delays = parse_delays(
                                synaptic_delay_params, postPop, synapseList
                            )
                        ...
                        if distance_dependence_params != None:
                            dist_par = parse_distance_dependent_rule(
                                distance_dependence_params,
                                prePop,
                                postPop,
                                projInfo["Type"],
                            )
                        ...
                        PostSegLengthDict, cached_target_dict = check_cached_dicts(
                            postCellObject["PopObj"].component,
                            cached_target_dict,
                            target_comp_groups,
                            path_to_nml2=path_to_cells,
                        )
                        if pre_segment_group_info != []:
                            passed_pre_seg_groups = check_pre_segment_groups(
                                pre_segment_group_info
                            )

                            if not passed_pre_seg_groups:
                                opencortex.print_comment_v(
                                    "Error: the list pre_segment_group_info was not correctly specified. Execution will terminate."
                                )
                                ...
                                if len(pre_segment_group_info) == 1:
                                    if (
                                        pre_segment_group_info[0]["ProjType"]
                                        == projInfo["Type"]
                                    ):
                                        PreSegLengthDict, cached_target_dict = (
                                            check_cached_dicts(
                                                preCellObject["PopObj"].component,
                                                cached_target_dict,
                                                [
                                                    pre_segment_group_info[0][
                                                        "PreSegGroup"
                                                    ]
                                                ],
                                                path_to_nml2=path_to_cells,
                                ...
                                if len(pre_segment_group_info) > 1:
                                    ...
                                    for proj in range(0, len(pre_segment_group_info)):
                                        check_pre_pop = (
                                            pre_segment_group_info[proj]["PrePop"]
                                            == prePop
                                        )

                                        check_post_pop = (
                                            pre_segment_group_info[proj]["PostPop"]
                                            == postPop
                                        )

                                        check_proj_type = (
                                            pre_segment_group_info[proj]["ProjType"]
                                            == projInfo["Type"]
                                        )
                        ...
                        compound_proj = build_projection(
                            net=net,
...
def build_probability_based_connectivity(
    net,
    pop_params,
    probability_matrix,
    synapse_matrix,
    ...
    passed_probs = check_matrix_size_and_type(
        matrix_of_params=probability_matrix,
        num_of_pop_tags=len(tags_on_populations),
        type_of_matrix=float,
    )
    ...
    passed_weights = check_matrix_size_and_type(
        matrix_of_params=weight_matrix,
        num_of_pop_tags=len(tags_on_populations),
        type_of_matrix=float,
    )
    ...
    passed_delays = check_matrix_size_and_type(
        matrix_of_params=delay_matrix,
        num_of_pop_tags=len(tags_on_populations),
        type_of_matrix=float,
    )
    ...
    if std_weight_matrix != None:
        passed_std_weights = check_matrix_size_and_type(
            matrix_of_params=std_weight_matrix,
            num_of_pop_tags=len(tags_on_populations),
            type_of_matrix=float,
        )
        ...
        if not passed_std_weights:
            ...
            matrices_with_errors.append(
                "matrix of standard deviations for synaptic weights"
            )

    if std_delay_matrix != None:
        passed_std_delays = check_matrix_size_and_type(
            matrix_of_params=std_delay_matrix,
            num_of_pop_tags=len(tags_on_populations),
            type_of_matrix=float,
        )
        ...
        if not passed_std_delays:
            ...
            matrices_with_errors.append(
                "matrix of standard deviations for synaptic delays"
            )

    if errors_found == 0:
        opencortex.print_comment_v(
            "All of the connectivity matrices were specified correctly."
        )

    else:
        opencortex.print_comment_v(
            "The connectivity matrices in %s were not specified correctly; execution will terminate."
            % matrices_with_errors
        )
    ...
    for pre_pop_id in pop_params.keys():
        for post_pop_id in pop_params.keys():
            ...
            if found_pre_pop and found_post_pop:
                ...
                ####################################################################
                prob_val = parse_parameter_value(
                    parameter_matrix=probability_matrix,
                    row_ind=row_index,
                    col_ind=column_index,
                    checks_passed=True,
                )

                synapse_list = parse_parameter_value(
                    parameter_matrix=synapse_matrix,
                    row_ind=row_index,
                    col_ind=column_index,
                    checks_passed=True,
                )
                ...
                if prob_val != 0 and pre_pop_obj.size != 0 and post_pop_obj.size != 0:
                    if synapse_list != None:
                        ...
                        ##########################################################################
                        weight_list = parse_parameter_value(
                            parameter_matrix=weight_matrix,
                            row_ind=row_index,
                            col_ind=column_index,
                            checks_passed=True,
                        )
                        ...
                        ###########################################################################
                        delay_list = parse_parameter_value(
                            parameter_matrix=delay_matrix,
                            row_ind=row_index,
                            col_ind=column_index,
                            checks_passed=True,
                        )
                        ...
                        ############################################################################
                        if std_weight_matrix != None:
                            std_weight_list = parse_parameter_value(
                                parameter_matrix=std_weight_matrix,
                                row_ind=row_index,
                                col_ind=column_index,
                                checks_passed=True,
                            )
                        ...
                        #############################################################################
                        if std_delay_matrix != None:
                            std_delay_list = parse_parameter_value(
                                parameter_matrix=std_delay_matrix,
                                row_ind=row_index,
                                col_ind=column_index,
                                checks_passed=True,
                            )
                        ...
                        returned_projs = oc_build.add_probabilistic_projection_list(
                            net=net,
                        ...
                        if returned_projs != None:
                            opencortex.print_comment_v(
                                "Addded a projection between %s and %s"
                        ...
                        opencortex.print_comment_v(
                            "Error in opencortex.utils.build_probability_based_connectivity(): connection probability is not equal to 0 but synapse list is None"
                            " for the projection between presynaptic population id=%s and postsynaptic population id= %s. Execution will terminate."
                            % (pre_pop_id, post_pop_id)
                        )
                ...
                if not found_pre_pop:
                    opencortex.print_comment_v(
                        "Error in build_probability_based_connectivity(): input argument tags_on_populations in build_probability_based_connectivity()"
                        " does not contain a string tag for the population %s. Execution will terminate."
                        % pre_pop_id
                    )
...
def read_connectivity(pre_pop, post_pop, path_to_txt_file, ignore_synapses=[]):
    ...
    for line in lines:
        ...
        if pre_pop in extract_info[0] and post_pop in extract_info[1]:
            ...
            for syn in ignore_synapses:
                if syn in synapse_list:
                    synapse_list.remove(syn)
...
def check_matrix_size_and_type(matrix_of_params, num_of_pop_tags, type_of_matrix):
    ...
    if isinstance(matrix_of_params, list):
        if len(matrix_of_params) == 1:
            if matrix_of_params[0] != None:
                if not isinstance(matrix_of_params[0], list):
                    if not isinstance(matrix_of_params[0], type_of_matrix):
                        opencortex.print_comment_v(
                            "Error in check_matrix_size(): argument matrix_of_params is a list which contains one element but it is not of type %s;"
                            " the current type is %s."
                            % (type_of_matrix, type(matrix_of_params[0]))
                        )
                    ...
                    for component_index in range(0, len(matrix_of_params[0])):
                        if not isinstance(
                            matrix_of_params[0][component_index], type_of_matrix
                        ):
                            opencortex.print_comment_v(
                                "Error in check_matrix_size(): argument matrix_of_params is a list which contains a list but not all of its elements are of type %s;"
                                " the current type is %s."
                                % (type_of_matrix, type(matrix_of_params[0][component]))
                            )
            ...
            for row_ind in range(0, len(matrix_of_params)):
                for col_ind in range(0, len(matrix_of_params[row_ind])):
                    if matrix_of_params[row_ind][col_ind] != None:
                        if not isinstance(matrix_of_params[row_ind][col_ind], list):
                            if not isinstance(
                                matrix_of_params[row_ind][col_ind], type_of_matrix
                            ):
                                opencortex.print_comment_v(
                                    "Error in check_matrix_size(): argument matrix_of_params is a list of lists (matrix) but there is an element inside which is not"
                                    " of type %s; the current type is %s."
                                    % (
                                        type_of_matrix,
                                        type(matrix_of_params[row_ind][col_ind]),
                                    )
                                )
                            ...
                            for component_index in range(
                                0, len(matrix_of_params[row_ind][col_ind])
                            ):
                                if not isinstance(
                                    matrix_of_params[row_ind][col_ind][component_index],
                                    type_of_matrix,
                                ):
                                    opencortex.print_comment_v(
                                        "Error in check_matrix_size(): argument matrix_of_params is a list of lists (matrix) but list elements inside are not"
                                        "of type %s; the current type is %s."
                                        % (
                                            type_of_matrix,
                                            type(
                                                matrix_of_params[row_ind][col_ind][
                                                    component_index
                                                ]
                                            ),
                                        )
                ...
                if len(matrix_of_params[row_ind]) != num_of_pop_tags:
                    opencortex.print_comment_v(
                        "Error in check_matrix_size(): each list element of the argument matrix_of_params must have length = %d;"
                        " the current length is %d."
                        % (num_of_pop_tags, len(matrix_of_params[row_ind]))
                    )
            ...
            if num_of_elements != num_of_pop_tags * num_of_pop_tags:
                opencortex.print_comment_v(
                    "Error in check_matrix_size(): argument matrix_of_params is a list but not in the format of an %d by %d matrix."
        ...
        if rows_cols[0] != num_of_pop_tags or rows_cols[1] != num_of_pop_tags:
            opencortex.print_comment_v(
                "Error in check_matrix_size(): argument matrix_of_params is a numpy array but not of size (%d, %d) (not a square matrix)."
                % num_of_pop_tags
            )
        ...
        opencortex.print_comment_v(
            "Error in check_matrix_size(): argument matrix_of_params must be a list or numpy array; the current type is %s."
...
def check_cached_dicts(
    cell_component, cached_dicts, list_of_target_seg_groups, path_to_nml2=None
):
    ...
    if cell_component in cached_dicts.keys():
        ...
        for target_group in list_of_target_seg_groups:
            if target_group not in cached_dicts[cell_component]["TargetDict"].keys():
                ...
                segLengthDict[target_group] = cached_dicts[cell_component][
                    "TargetDict"
                ][target_group]
            ...
            if new_segment_groups:
                ...
                target_segments = oc_build.extract_seg_ids(
                    cell_object=cellObject,
                    target_compartment_array=target_groups_to_include,
                    targeting_mode="segGroups",
                )

                new_seg_length_dict = oc_build.make_target_dict(
                    cell_object=cellObject, target_segs=target_segments
                )

                for new_target_group in new_seg_length_dict.keys():
                    cached_dicts[cell_component]["TargetDict"][new_target_group] = (
                        new_seg_length_dict[new_target_group]
                    )

                    segLengthDict[new_target_group] = new_seg_length_dict[
                        new_target_group
                    ]
        ...
        if path_to_nml2 != None:
            document_cell = neuroml.loaders.NeuroMLLoader.load(
                os.path.join(path_to_nml2, cell_nml_file)
            )
        ...
        target_segments = oc_build.extract_seg_ids(
            cell_object=cellObject,
            target_compartment_array=list_of_target_seg_groups,
            targeting_mode="segGroups",
        )

        segLengthDict = oc_build.make_target_dict(
            cell_object=cellObject, target_segs=target_segments
        )
    ...
    if len(pre_segment_group_info) == 1:
        if isinstance(pre_segment_group_info[0], dict):
            ...
            if not check_pre_seg:
                opencortex.print_comment_v(
                    "Error in build connectivity: the key 'PreSegGroup' is not in the dictionary keys inside pre_segment_group_info."
                )
                error_counter += 1
                ...
                if not isinstance(pre_segment_group_info[0]["PreSegGroup"], str):
                    opencortex.print_comment_v(
                        "Error in build connectivity: the value of the key 'PreSegGroup' in the dictionary keys inside pre_segment_group_info"
            ...
            if not check_proj_type:
                opencortex.print_comment_v(
                    "Error in build connectivity: the key 'ProjType' is not in the dictionary keys inside pre_segment_group_info."
                )
                error_counter += 1

        else:
            opencortex.print_comment_v(
                "Error in build_connectivity: the list pre_segment_group_info has only one element but it is not of type 'dict'."
        ...
        for proj in range(0, len(pre_segment_group_info)):
            if isinstance(pre_segment_group_info[proj], dict):
                ...
                if not check_pre_pop:
                    opencortex.print_comment_v(
                        "Error in build connectivity: the key 'PrePop' is not in the dictionary keys inside pre_segment_group_info."
                    )
                    error_counter += 1

                if not check_post_pop:
                    opencortex.print_comment_v(
                        "Error in build connectivity: the key 'PostPop' is not in the dictionary keys inside pre_segment_group_info."
                    )
                    error_counter += 1
                ...
                opencortex.print_comment_v(
                    "Error in build_connectivity: the list elements in the pre_segment_group_info must be dictionaries with fields"
                    " 'PreSegGroup', 'PrePop', 'PostPop' and 'ProjType'. The current type is %s."
                    % (type(pre_segment_group_info[proj]))
                )
                error_counter += 1
...
def build_inputs(
    ...
    passed_inputs = check_inputs(
        input_params, population_params, path_to_cells, path_to_synapses
    )
    ...
    if passed_inputs:
        ...
        opencortex.print_comment_v(
            "Input parameters were specified incorrectly; execution will terminate."
        )
            ...
            if cell_tag in cell_population:
                ...
                for input_group_ind in range(0, len(input_params[cell_tag])):
                    ...
                    if not input_group_params["LocationSpecific"]:
                        target_cell_ids = oc_build.get_target_cells(
                            population=pop, fraction_to_target=fraction_to_target
                        )
                    ...
                    if target_cell_ids != []:
                        ...
                        condition2 = (
                            "UniversalTargetSegmentID" not in input_group_params.keys()
                        )

                        condition3 = (
                            "UniversalFractionAlong" not in input_group_params.keys()
                        )
                        ...
                        if condition1 and condition2 and condition3:
                            ...
                            if None not in input_group_params["TargetDict"].keys():
                                ...
                                if cached_dicts != None:
                                    segLengthDict, cached_dicts = check_cached_dicts(
                                        cell_component,
                                        cached_dicts,
                                        target_group_list,
                                        path_to_nml2=path_to_cells,
                                    ...
                                    target_segments = oc_build.extract_seg_ids(
                                        cell_object=cellObject,
                                        target_compartment_array=input_group_params[
                                            "TargetDict"
                                        ].keys(),
                                        targeting_mode="segGroups",
                                    )
                            ...
                            target_segment = input_group_params[
                                "UniversalTargetSegmentID"
                            ]

                            fraction_along = input_group_params[
                                "UniversalFractionAlong"
                            ]
                        ...
                        if input_group_params["InputType"] == "GeneratePoissonTrains":
                            ...
                            if input_group_params["TrainType"] == "transient":
                                for input_index in range(
                                    0, len(input_group_params["AverageRateList"])
                                ):
                                    tpfs = (
                                        oc_build._add_transient_poisson_firing_synapse(
                                            nml_doc=nml_doc,
                                            id=input_group_tag
                                            + "_TransPoiSyn%d" % input_index,
                                            average_rate="%f %s"
                                            % (
                                                input_group_params["AverageRateList"][
                                                    input_index
                                                ],
                                                input_group_params["RateUnits"],
                                            ),
                                            delay="%f %s"
                                            % (
                                                input_group_params["DelayList"][
                                                    input_index
                                                ],
                                                input_group_params["TimeUnits"],
                                            ),
                                            duration="%f %s"
                                            % (
                                                input_group_params["DurationList"][
                                                    input_index
                                                ],
                                                input_group_params["TimeUnits"],
                                            ),
                                    ...
                                    input_synapse_list.append(
                                        input_group_params["Synapse"]
                                    )
                            ...
                            if input_group_params["TrainType"] == "persistent":
                                for input_index in range(
                                    ...
                                    pfs = oc_build._add_poisson_firing_synapse(
                                        nml_doc=nml_doc,
                        ...
                        if input_group_params["InputType"] == "PulseGenerators":
                            if not input_group_params["Noise"]:
                                ...
                                for input_index in range(
                                    0, len(input_group_params["AmplitudeList"])
                                ):
                                    pg = oc_build._add_pulse_generator(
                                        nml_doc=nml_doc,
                                        ...
                                        amplitude="%f %s"
                                        % (
                                            input_group_params["AmplitudeList"][
                                                input_index
                                            ],
                                            input_group_params["AmplitudeUnits"],
                                        ),
                                    )
                                ...
                                for cell_id in target_cell_ids:
                                    assert (
                                        len(input_group_params["SmallestAmplitudeList"])
                                        == 1
                                    )
                                    ...
                                    for input_index in range(
                                        0,
                                        len(
                                            input_group_params["SmallestAmplitudeList"]
                                        ),
                                    ):
                                        random_amplitude = random.uniform(
                                            input_group_params["SmallestAmplitudeList"][
                                                input_index
                                            ],
                                ...
                                pg = oc_build._add_pulse_generator(
                                    ...
                                    delay="%s %s"
                                    % (
                                        input_group_params["DelayList"][0],
                                        input_group_params["TimeUnits"],
                                    ),
                                    duration="%s %s"
                                    % (
                                        input_group_params["DurationList"][0],
                                        input_group_params["TimeUnits"],
                                    ),
                                    amplitude="%s %s"
                                    % (1, input_group_params["AmplitudeUnits"]),
                                )
                        ...
                        if (
                            input_group_params["InputType"]
                            == "GenerateSpikeSourcePoisson"
                        ):
                            list_of_input_ids = []
                            ...
                            for input_index in range(
                                ...
                                ssp = oc_build._add_spike_source_poisson(
                                    nml_doc=nml_doc,
                                    id=input_group_tag + "_SpSourcePoi%d" % input_index,
                                    start="%f %s"
                                    % (
                                        input_group_params["DelayList"][input_index],
                                        input_group_params["TimeUnits"],
                                    ),
                                    duration="%f %s"
                                    % (
                                        input_group_params["DurationList"][input_index],
                                        input_group_params["TimeUnits"],
                                    ),
                                    rate="%f %s"
                                    % (
                                ...
                                weight_list.append(
                                    input_group_params["WeightList"][input_index]
                                )
                        ...
                        if (
                            input_group_params["InputType"] == "GeneratePoissonTrains"
                            or input_group_params["InputType"] == "PulseGenerators"
                        ):
                            input_list_array = (
                                oc_build.add_advanced_inputs_to_population(
                                    net=net,
                                    id=input_group_tag,
                                    population=pop,
                        ...
                        if (
                            ...
                            input_list_array = oc_build.add_projection_based_inputs(
                                net=net,
                                id=input_group_tag,
                                population=pop,
                                input_id_list=input_ids_final,
...
def replace_cell_types(
    ...
    if len(cell_types_to_be_replaced) == len(cell_types_replaced_by):
        ...
        if (not reduced_to_single_compartment) and (
            connection_segment_groups != None or input_segment_groups != None
        ):
            cached_target_dict = {}
            ...
            for cell_index in range(0, len(cell_types_replaced_by)):
                if pop.component == cell_types_to_be_replaced[cell_index]:
                    ...
                    if cell_types_to_be_replaced[cell_index] in pop.id:
                        pop.id = pop.id.replace(
                            cell_types_to_be_replaced[cell_index],
                            cell_types_replaced_by[cell_index],
                        )
        ...
        for proj_group_index in range(0, len(all_projections_final)):
            ...
            for proj_counter in range(0, len(projections)):
                ...
                for conversion_index in range(0, len(old_to_new)):
                    ...
                    if proj.presynaptic_population == conversion_params["OldPopID"]:
                        ...
                        if proj.presynaptic_population in proj.id:
                            proj.id = proj.id.replace(
                                proj.presynaptic_population,
                                conversion_params["NewPopID"],
                            )
                ...
                if (
                    not reduced_to_single_compartment
                ) and connection_segment_groups != None:
                    if replaced_pre_pop and replaced_post_pop:
                        for index in range(0, len(connection_segment_groups)):
                            ...
                            check_pre_cell_type = (
                                old_to_new[pre_pop_index]["NewCellComponent"]
                                == proj_info["PreCellType"]
                            )

                            check_post_cell_type = (
                                old_to_new[post_pop_index]["NewCellComponent"]
                                == proj_info["PostCellType"]
                            )
                            ...
                            if (
                                check_pre_cell_type
                                and check_post_cell_type
                                and check_proj_type
                            ):
                                pre_seg_length_dict, cached_target_dict = (
                                    check_cached_dicts(
                                        old_to_new[pre_pop_index]["NewCellComponent"],
                                        cached_target_dict,
                                ...
                                post_seg_length_dict, cached_target_dict = (
                                    check_cached_dicts(
                                        old_to_new[post_pop_index]["NewCellComponent"],
                                        cached_target_dict,
                ...
                for conn_counter in range(0, len(connections)):
                    ...
                    if replaced_post_pop:
                        if id_tag:
                            if (
                                old_to_new[post_pop_index]["OldCellComponent"]
                                in connection.post_cell_id
                            ):
                                connection.post_cell_id = (
                                    connection.post_cell_id.replace(
                                        old_to_new[post_pop_index]["OldCellComponent"],
                                        old_to_new[post_pop_index]["NewCellComponent"],
                                    )
                                )
                            ...
                            if (
                                old_to_new[post_pop_index]["OldPopID"]
                                in connection.post_cell_id
                            ):
                                connection.post_cell_id = (
                                    connection.post_cell_id.replace(
                                        old_to_new[post_pop_index]["OldPopID"],
                                        old_to_new[post_pop_index]["NewPopID"],
                                    )
                                )
                            ...
                            if (
                                old_to_new[post_pop_index]["OldCellComponent"]
                                in connection.post_cell
                            ):
                                connection.post_cell = connection.post_cell.replace(
                                    old_to_new[post_pop_index]["OldCellComponent"],
                                    old_to_new[post_pop_index]["NewCellComponent"],
                                )
                        ...
                        if post_seg_length_dict != None and post_subset_dict != None:
                            post_target_seg_array, post_target_fractions = (
                                oc_build.get_target_segments(
                                    post_seg_length_dict, post_subset_dict
                                )
                            )
                    ...
                    if replaced_pre_pop:
                        if id_tag:
                            if (
                                old_to_new[pre_pop_index]["OldCellComponent"]
                                in connection.pre_cell_id
                            ):
                                connection.pre_cell_id = connection.pre_cell_id.replace(
                                    old_to_new[pre_pop_index]["OldCellComponent"],
                                    old_to_new[pre_pop_index]["NewCellComponent"],
                                )

                            if (
                                old_to_new[pre_pop_index]["OldPopID"]
                                in connection.pre_cell_id
                            ):
                                connection.pre_cell_id = connection.pre_cell_id.replace(
                                    old_to_new[pre_pop_index]["OldPopID"],
                                    old_to_new[pre_pop_index]["NewPopID"],
                                )
                        else:
                            if (
                                old_to_new[pre_pop_index]["OldCellComponent"]
                                in connection.pre_cell
                            ):
                                connection.pre_cell = connection.pre_cell.replace(
                                    old_to_new[pre_pop_index]["OldCellComponent"],
                                    old_to_new[pre_pop_index]["NewCellComponent"],
                                )
                        ...
                        if pre_seg_length_dict != None and pre_subset_dict != None:
                            pre_target_seg_array, pre_target_fractions = (
                                oc_build.get_target_segments(
                                    pre_seg_length_dict, pre_subset_dict
                                )
                            )
        ...
        for input_list_index in range(0, len(net.input_lists)):
            ...
            for conversion_index in range(0, len(old_to_new)):
                ...
                if conversion_params["OldPopID"] == input_list_obj.populations:
                    ...
                    if (
                        not reduced_to_single_compartment
                    ) and input_segment_groups != None:
                        for index in range(0, len(input_segment_groups)):
                            input_group_info = input_segment_groups[index]

                            check_post_cell_type = (
                                conversion_params["NewCellComponent"]
                                == input_group_info["PostCellType"]
                            )

                            if check_post_cell_type:
                                target_segment_dict, cached_target_dict = (
                                    check_cached_dicts(
                                        conversion_params["NewCellComponent"],
                                        cached_target_dict,
                    ...
                    for input_index in range(0, len(input_list_obj.input)):
                        ...
                        if conversion_params["OldPopID"] in input_obj.target:
                            input_obj.target = input_obj.target.replace(
                                conversion_params["OldPopID"],
                                conversion_params["NewPopID"],
                            )

                        if conversion_params["OldCellComponent"] in input_obj.target:
                            input_obj.target = input_obj.target.replace(
                                conversion_params["OldCellComponent"],
                                conversion_params["NewCellComponent"],
                            )
                        ...
                        if require_segment_groups:
                            if input_obj.segment_id != 0:
                                target_seg_array, target_fractions = (
                                    oc_build.get_target_segments(
                                        target_segment_dict, target_subset_dict
                                    )
                                )
        ...
        for changed_cell in range(0, len(old_to_new)):
            ...
            replaced_components_final.append(
                old_to_new[changed_cell]["OldCellComponent"]
            )

            oc_build.add_cell_and_channels(
                net_doc,
                os.path.join(dir_to_new_components, "%s.cell.nml" % cell_model),
                cell_model,
        ...
        for cell_model in all_old_components:
            if cell_model not in replaced_components_final:
                oc_build.add_cell_and_channels(
                    net_doc,
                    os.path.join(dir_to_old_components, "%s.cell.nml" % cell_model),
                    cell_model,
                    use_prototypes=False,
                )
        ...
        opencortex.print_comment_v(
            "Error: the number of cell types in the list cell_types_to_be_replaced is not equal to the number of new cell types in the list"
            " cell_types_replaced_by."
        )
...
def parse_distance_dependence_params(
    distance_dependence_params, pre_pop, post_pop, proj_type
):
    dist_rule = None

    for distance_param in range(0, len(distance_dependence_params)):
        check_pre_pop = (
            distance_dependence_params[distance_param]["PrePopID"] == pre_pop
        )

        check_post_pop = (
            distance_dependence_params[distance_param]["PostPopID"] == post_pop
        )

        check_proj_type = (
            distance_dependence_params[distance_param]["Type"] == proj_type
        )
    ...
    for syn_ind in range(0, len(synapse_list)):
        for delay_param in range(0, len(delays_params)):
            if delays_params[delay_param]["synComp"] == "all":
                delays[synapse_list[syn_ind]] = float(
                    delays_params[delay_param]["delay"]
                )
                ...
                if delays_params[delay_param]["targetCellGroup"] == []:
                    ...
                    for target_cell_group in delays_params[delay_param][
                        "targetCellGroup"
                    ]:
    ...
    for syn_ind in range(0, len(synapse_list)):
        for weight_param in range(0, len(weights_params)):
            if weights_params[weight_param]["synComp"] == "all":
                weights[synapse_list[syn_ind]] = float(
                    weights_params[weight_param]["weight"]
                )
                ...
                if weights_params[weight_param]["targetCellGroup"] == []:
                    ...
                    for target_cell_group in weights_params[weight_param][
                        "targetCellGroup"
                    ]:
                        if target_cell_group in post_pop:
                            passed_targetCellGroup = True
...
def check_includes_in_cells(dir_to_cells, list_of_cell_ids, extra_channel_tags=None):
    passed = True
    ...
    for cell_file_name in target_files:
        ...
        if not os.path.exists(full_path_to_cell):
            ...
            opencortex.print_comment_v(
                "Error: path %s does not exist; use method copy_nml2_source to copy nml2 files from the source directory to the appropriate NeuroML2 component directories."
                % full_path_to_cell
            )
            ...
            nml2_doc_cell = pynml.read_neuroml2_file(
                full_path_to_cell, include_includes=False
            )

            for included in nml2_doc_cell.includes:
                if ".channel.nml" in included.href:
                    if ("../channels/" not in included.href) or (
                        "..\channels'" not in included.href
                    ):
                        ...
                        for channel_tag in included.href:
                            if channel_tag in included.href:
                                if ("../channels/" not in included.href) or (
                                    ...
                                    included.href = os.path.join(
                                        channel_dir, included.href
                                    )
...
def check_pop_dict_and_layers(pop_dict, boundary_dict):
    ...
    for cell_population in pop_dict.keys():
        if not isinstance(pop_dict[cell_population], tuple):
            print(
                "TypeError in population parameters: the values stored in the population dictionary must be tuples."
            )
            ...
            if len(pop_dict[cell_population]) != 5:
                print(
                    "ValueError in population parameters: tuples in the population dictionary must contain four elements in the following order and type: "
                ...
                if not isinstance(pop_dict[cell_population][0], int):
                    print(
                        "TypeError in population parameters: the first element in tuples in the population dictionary must be of type 'int'"
                    )
                    print(
                        " as it specifies the size of cell population. The current type of the first element is %s"
                        % (type(pop_dict[cell_population][0]))
                    )
                    error_counter += 1

                if not isinstance(pop_dict[cell_population][1], str):
                    print(
                        "TypeError in population parameters: the second element in tuples in the population dictionary must be of type 'string'"
                    )
                    print(
                        " as it specifies the layer of cell population. The current type of the second element is %s"
                        % (type(pop_dict[cell_population][1]))
                    )
                    error_counter += 1
                    ...
                    try:
                        ...
                        print(
                            "KeyError in the layer boundary dictionary: cell population id '%s' is not in the keys of the layer boundary dictionary"
                            % cell_population
                        )
                        error_counter += 1
                ...
                if not isinstance(pop_dict[cell_population][2], str):
                    print(
                        "TypeError in population parameters: the third element in tuples in the population dictionary must be of type 'string'"
                    )
                    print(
                        " as it specifies the cell model for a given population. The current type of the third element is %s"
                        % (type(pp_dict[cell_population][2]))
                    )
                    error_counter += 1

                if not isinstance(pop_dict[cell_population][3], str):
                    print(
                        "TypeError in population parameters: the fourth element in tuples in the population dictionary must be of type 'string'"
                    )
                    print(
                    ...
                    if not (
                        pop_dict[cell_population][3] == "single"
                        or pop_dict[cell_population][3] == "multi"
                    ):
                        print(
                            "ValueError in population parameters: the fourth element in tuples in the population dictionary must be equal to 'single' or 'multi'"
                            " as it specifies the compartmentalization for a given cell type. The current type of the fourth element is %s"
                        )
                        error_counter += 1
...
def check_weight_params(weight_params):
    ...
    if not isinstance(weight_params, list):
        print(
            "TypeError in weight parameters: weight parameters must be of type 'list'. The current type is '%s'."
            % (type(weight_params))
        )
        ...
        for weight_param in range(0, len(weight_params)):
            if not isinstance(weight_params[weight_param], dict):
                print(
                    "TypeError in weight parameters: list elements in weight parameters must be of type 'dict'. The current type is '%s'."
                    % (type(weight_params[weight_param]))
                )
                ...
                try:
                    ...
                    print(
                        "KeyError in weight parameters: the key 'weight' is not in the keys of weight parameter dictionary."
                    )
                ...
                try:
                    ...
                    if not isinstance(test_syn_comp_field, str):
                        print(
                            "TypeError in weight parameters: the value of the key 'synComp' must be of type 'str'. The current type is '%s'."
                            % (type(test_syn_comp_field))
                        )
                        ...
                        if test_syn_comp_field != "all":
                            try:
                                test_syn_ends_with_field = weight_params[weight_param][
                                    "synEndsWith"
                                ]

                                if not isinstance(test_syn_ends_with_field, list):
                                    print(
                                        "TypeError in weight parameters: the value of the key 'synEndsWith' must be of type 'list'."
                                    )
                                    print(
                                        "The current type is '%s'."
                                        % (type(test_syn_ends_with_field))
                                    )
                                ...
                                print(
                                    "KeyError in weight parameters: the key 'synEndsWith' is not in the keys of weight parameter dictionary."
                                )

                                error_counter += 1

                            try:
                                test_target_cell_group = weight_params[weight_param][
                                    "targetCellGroup"
                                ]

                                if not isinstance(test_target_cell_group, list):
                                    print(
                                        "TypeError in weight parameters: the value of the key 'targetCellGroup' must be of type 'list'."
                                    )
                                    print(
                                        "The current type is '%s'."
                                        % (type(test_target_cell_group))
                                    )
                                ...
                                print(
                                    "KeyError in weight parameters: the key 'targetCellGroup' is not in the keys of weight parameter dictionary."
                                )
                    ...
                    print(
                        "KeyError in weight parameters: the key 'synComp' is not in the keys of weight parameter dictionary."
                    )
...
def check_delay_params(delay_params):
    ...
    if not isinstance(delay_params, list):
        print(
            "TypeError in delay parameters: delay parameters must be of type 'list'. The current type is '%s'."
            % (type(delay_params))
        )
        ...
        for delay_param in range(0, len(delay_params)):
            if not isinstance(delay_params[delay_param], dict):
                print(
                    "TypeError in delay parameters: list elements in delay parameters must be of type 'dict'. The current type is '%s'."
                    % (type(delay_params[delay_param]))
                )
                ...
                try:
                    ...
                    print(
                        "KeyError in delay parameters: the key 'delay' is not in the keys of delay parameter dictionary."
                    )
                ...
                try:
                    ...
                    if not isinstance(test_syn_comp_field, str):
                        print(
                            "TypeError in delay parameters: the value of the key 'synComp' must be of type 'str'. The current type is '%s'."
                            % (type(test_syn_comp_field))
                        )
                        ...
                        if test_syn_comp_field != "all":
                            try:
                                test_syn_ends_with_field = delay_params[delay_param][
                                    "synEndsWith"
                                ]

                                if not isinstance(test_syn_ends_with_field, list):
                                    print(
                                        "TypeError in delay parameters: the value of the key 'synEndsWith' must be of type 'list'."
                                    )
                                    print(
                                        "The current type is '%s'."
                                        % (type(test_syn_ends_with_field))
                                ...
                                print(
                                    "KeyError in delay parameters: the key 'synEndsWith' is not in the keys of delay parameter dictionary."
                                )

                                error_counter += 1

                            try:
                                test_target_cell_group = delay_params[delay_param][
                                    "targetCellGroup"
                                ]

                                if not isinstance(test_target_cell_group, list):
                                    print(
                                        "TypeError in delay parameters: the value of the key 'targetCellGroup' must be of type 'list'."
                                    )
                                    print(
                                        "The current type is '%s'."
                                        % (type(test_target_cell_group))
                                ...
                                print(
                                    "KeyError in delay parameters: the key 'targetCellGroup' is not in the keys of delay parameter dictionary."
                                )
                    ...
                    print(
                        "KeyError in delay parameters: the key 'synComp' is not in the keys of delay parameter dictionary."
                    )
...
def check_inputs(input_params, popDict, path_to_cells, path_to_synapses=None):
    ...
    for cell_receiver in input_params.keys():
            ...
            if cell_receiver in pop_id:
                ...
                if test_cell_component["Compartments"] == "multi":
                    segment_groups = get_segment_groups(
                        test_cell_component["PopObj"].component, path_to_cells
                    )
        ...
        if not found_target_pop:
            opencortex.print_comment_v(
                "KeyError in input parameters: cell population id '%s' is not in the keys of population dictionary"
                % cell_receiver
            )
        ...
        if not isinstance(input_params[cell_receiver], list):
            opencortex.print_comment_v(
                "TypeError in input parameters: the dictionary value for '%s' must be a list."
                " The current type is %s"
                % (cell_receiver, type(input_params[cell_receiver]))
            )
            ...
            for input_group_ind in range(0, len(input_params[cell_receiver])):
                ...
                try:
                    ...
                    if not isinstance(test_key, str):
                        opencortex.print_comment_v(
                            "TypeError in input parameters: the value of the key 'InputType' must be of type 'string'. The current type is %s"
                            % type(test_key)
                        )
                    ...
                    if test_key not in [
                        ...
                        opencortex.print_comment_v(
                            "ValueError in input parameters: the value of the key 'InputType' must be one of the following: "
                        ...
                        if test_key == "GeneratePoissonTrains":
                            try:
                                ...
                                if not isinstance(test_train_type, str):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'TrainType' must be of type 'string'. "
                                        "The current type is %s" % type(test_train_type)
                                    )

                                    error_counter += 1
                                else:
                                    if test_train_type not in [
                                        "persistent",
                                        "transient",
                                    ]:
                                        opencortex.print_comment_v(
                                            "ValueError in input parameters: the value of the key 'TrainType' when 'InputType' is 'GeneratePoissonTrains' must be"
                                        ...
                                        if test_train_type == "persistent":
                                            try:
                                                test_rates = input_group_params[
                                                    "AverageRateList"
                                                ]

                                                if not isinstance(test_rates, list):
                                                    opencortex.print_comment_v(
                                                        "TypeError in input parameters: the value of the key 'AverageRateList' must be of type 'list'."
                                                        " The current type is %s"
                                                        % type(test_rates)
                                                    )
                                                    ...
                                                    for r in range(0, len(test_rates)):
                                                        if not isinstance(
                                                            ...
                                                            opencortex.print_comment_v(
                                                                "TypeError in input parameters: the list values of the key 'AverageRateList' must be of type 'float'."
                                                                " The current type is %s"
                                                                % type(test_rates[r])
                                                            )
                                                ...
                                                opencortex.print_comment_v(
                                                    "KeyError in input parameters: the key 'AverageRateList' is not in the keys of input parameters."
                                                )
                                        ...
                                        if test_train_type == "transient":
                                            try:
                                                test_time_units = input_group_params[
                                                    "TimeUnits"
                                                ]

                                                if not isinstance(test_time_units, str):
                                                    opencortex.print_comment_v(
                                                        "TypeError in input parameters: the value of the key 'TimeUnits' must be of type 'str'."
                                                        " The current type is %s."
                                                        % type(test_time_units)
                                                    )
                                                ...
                                                opencortex.print_comment_v(
                                                    "KeyError in input parameters: the key 'TimeUnits' is not in the keys of input parameters."
                                                )
                                            ...
                                            try:
                                                ...
                                                if not isinstance(test_rates, list):
                                                    opencortex.print_comment_v(
                                                        "TypeError in input parameters: the value of the key 'DelayList' must be of type 'list'."
                                                        " The current type is %s."
                                                        % type(test_rates)
                                                    )
                                                    ...
                                                    for r in range(0, len(test_rates)):
                                                        if not isinstance(
                                                            ...
                                                            opencortex.print_comment_v(
                                                                "TypeError in input parameters: the list values of the key 'DelayList' must be of type 'float'."
                                                                " The current type is %s."
                                                                % type(test_rates[r])
                                                            )
                                                ...
                                                opencortex.print_comment_v(
                                                    "KeyError in input parameters: the key 'DelayList' is not in the keys of input parameters."
                                                )
                                            ...
                                            try:
                                                ...
                                                if not isinstance(test_rates, list):
                                                    opencortex.print_comment_v(
                                                        "TypeError in input parameters: the value of the key 'DurationList' must be of type 'list'."
                                                        " The current type is %s."
                                                        % type(test_rates)
                                                    )
                                                    ...
                                                    for r in range(0, len(test_rates)):
                                                        if not isinstance(
                                                            ...
                                                            opencortex.print_comment_v(
                                                                "TypeError in input parameters: the list values of the key 'DurationList' must be of type 'float'."
                                                                " The current type is %s."
                                                                % type(test_rates[r])
                                                            )
                                                ...
                                                opencortex.print_comment_v(
                                                    "KeyError in input parameters: the key 'DurationList' is not in the keys of input parameters."
                                                )
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parameters: the key 'TrainType' is not in the keys of input parameters."
                                )
                            ...
                            try:
                                ...
                                if not isinstance(test_rate_units, str):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'RateUnits' must be of type 'str'."
                                        "The current type is %s."
                                        % type(test_rate_units)
                                    )
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parametres: the key 'RateUnits' is not in the keys of inputs parameters."
                                )
                            ...
                            try:
                                ...
                                if not isinstance(test_synapse, str):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'Synapse' must be of type 'str'."
                                    ...
                                    if path_to_synapses != None:
                                        found = check_synapse_location(
                                            test_synapse, path_to_synapses
                                        )

                                        if not found:
                                            opencortex.print_comment_v(
                                                "ValueError in input parameters: the value '%s' of the key 'Synapse' is not found in %s"
                                                % (test_synapse, path_to_synapses)
                                            )
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parameters: the key 'Synapse' is not in the keys of input parameters."
                                )
                        ...
                        if test_key == "GenerateSpikeSourcePoisson":
                            ...
                            try:
                                ...
                                if not isinstance(test_weights, list):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'WeightList' must be of type 'list'."
                                    ...
                                    for r in range(0, len(test_weights)):
                                        if not isinstance(test_weights[r], float):
                                            opencortex.print_comment_v(
                                                "TypeError in input parameters: the list values of the key 'WeightList' must be of type 'float'."
                                                " The current type is %s."
                                                % type(test_weights[r])
                                            )
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parameters: the key 'WeightList' is not in the keys of input parameters."
                                )
                        ...
                        if test_key == "PulseGenerators":
                            ...
                            try:
                                test_amplitude_units = input_group_params[
                                    "AmplitudeUnits"
                                ]

                                if not isinstance(test_amplitude_units, str):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'AmplitudeUnits' must be of type 'str'."
                                        " The current type is %s."
                                        % type(test_amplitude_units)
                                    )
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parametres: the key 'AmplitudeUnits' is not in the keys of inputs parameters."
                                )
                            ...
                            try:
                                ...
                                if not isinstance(test_noise, bool):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'Noise' must be of type 'bool'."
                                    ...
                                    if test_noise:
                                        try:
                                            ...
                                            if not isinstance(
                                                test_smallest_amplitudes, list
                                            ):
                                                opencortex.print_comment_v(
                                                    "TypeError in input parameters: the value of the key 'SmallestAmplitudeList' must be of type 'list'."
                                                    " The current type is %s."
                                                    % type(test_smallest_amplitudes)
                                                )
                                            ...
                                            opencortex.print_comment_v(
                                                "KeyError in input parameters: the key 'SmallestAmplitudeList' is not in the keys of input parameters when "
                                        ...
                                        try:
                                            ...
                                            if not isinstance(
                                                test_largest_amplitudes, list
                                            ):
                                                opencortex.print_comment_v(
                                                    "TypeError in input parameters: the value of the key 'LargestAmplitudeList' must be of type 'list'."
                                                    " The current type is %s."
                                                    % type(test_largest_amplitudes)
                                                )
                                            ...
                                            opencortex.print_comment_v(
                                                "KeyError in input parameters: the key 'LargestAmplitudeList' is not in the keys of input parameters when "
                                                "'Noise' is set to True."
                                            )
                                        ...
                                        try:
                                            test_amplitudes = input_group_params[
                                                "AmplitudeList"
                                            ]

                                            if not isinstance(test_amplitudes, list):
                                                opencortex.print_comment_v(
                                                    "TypeError in input parameters: the value of the key 'AmplitudeList' must be of type 'list'."
                                                    " The current type is %s."
                                                    % type(test_amplitudes)
                                                )
                                                ...
                                                for r in range(0, len(test_amplitudes)):
                                                    if not isinstance(
                                                        test_amplitudes[r], float
                                                    ):
                                                        opencortex.print_comment_v(
                                                            "TypeError in input parameters: the list values of the key 'AverageRateList' must be of type 'float'."
                                                            " The current type is %s."
                                                            % type(test_amplitudes[r])
                                                        )
                                            ...
                                            opencortex.print_comment_v(
                                                "KeyError in input parameters: the key 'AmplitudeList' is not in the keys of input parameters."
                                            )
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parameters: the key 'Noise' is not in the keys of input parameters."
                                )
                    ...
                    opencortex.print_comment_v(
                        "KeyError in input parameters: the key 'InputType' is not in input parameters."
                    )
                ...
                try:
                    ...
                    if not isinstance(test_input_name, str):
                        opencortex.print_comment_v(
                            "TypeError in input parameters: the value of the key 'InputName' must be of type 'str'."
                    ...
                    opencortex.print_comment_v(
                        "KeyError in input parameters: the key 'InputName' is not in input parameters."
                    )

                    error_counter += 1

                if ("UniversalTargetSegmentID" not in input_group_params.keys()) and (
                    "UniversalFractionAlong" not in input_group_params.keys()
                ):
                    try:
                        ...
                        if not isinstance(test_key, dict):
                            opencortex.print_comment_v(
                                "TypeError in input parameters: the value of the key 'TargetDict' in input parameters must be of type 'dict'."
                            ...
                            if cell_type != None:
                                for target_segment_group in test_key.keys():
                                    if not check_segment_group(
                                        segment_groups, target_segment_group
                                    ):
                                        opencortex.print_comment_v(
                                            "ValueError in input parameters: '%s' is not a segment group of the cell type '%s'"
                                            % (target_segment_group, cell_receiver)
                                        )
                                        ...
                                        if not isinstance(
                                            test_key[target_segment_group], int
                                        ):
                                            opencortex.print_comment_v(
                                                "TypeError in input parameters: the value of the key '%s' must be of type 'int'."
                                                " The current type is %s"
                                                % (
                                                    target_segment_group,
                                                    type(
                                                        test_key[target_segment_group]
                                                    ),
                                                )
                                            )
                        ...
                        opencortex.print_comment_v(
                            "KeyError in input parameters: the key 'TargetDict' is not in input parameters."
                        )
                ...
                if "TargetDict" in input_group_params.keys():
                    if "UniversalTargetSegmentID" in input_group_params.keys():
                        opencortex.print_comment_v(
                            "KeyError in input parameters: the key 'UniversalTargetSegmentID' cannot be specified together with the key 'TargetDict'."
                        )
                    ...
                    if "UniversalFractionAlong" in input_group_params.keys():
                        opencortex.print_comment_v(
                            "KeyError in input parameters: the key 'UniversalFractionAlong' cannot be specified together with the key 'TargetDict'."
                        )
                    ...
                    try:
                        test_target_seg_id = input_group_params[
                            "UniversalTargetSegmentID"
                        ]

                    except KeyError:
                        opencortex.print_comment_v(
                            "KeyError in input parameters: the key 'UniversalTargetSegmentID' must be specified when the key 'TargetDict' is not in "
                            "input parameters."
                        )
                    ...
                    try:
                        test_fraction_along = input_group_params[
                            "UniversalFractionAlong"
                        ]

                    except KeyError:
                        opencortex.print_comment_v(
                            "KeyError in input parameters: the key 'UniversalFractionAlong' must be specified when the key 'TargetDict' is not in "
                            "input parameters."
                        )
                ...
                try:
                    ...
                    if not isinstance(test_key, float):
                        opencortex.print_comment_v(
                            "TypeError: the value of the key 'FractionToTarget' must be of type 'float'. The current type is %s."
                            % type(test_key)
                        )
                    ...
                    opencortex.print_comment_v(
                        "KeyError: the key 'FractionToTarget' is not in input parameters."
                    )
                ...
                try:
                    ...
                    if not isinstance(test_key, bool):
                        opencortex.print_comment_v(
                            "TypeError in input parameters: the value of the key 'LocationSpecific' must be of the type 'bool'."
                            " The current type is %s." % type(test_key)
                        )
                        ...
                        if test_key:
                            try:
                                ...
                                if not isinstance(test_region_key, list):
                                    opencortex.print_comment_v(
                                        "TypeError in input parameters: the value of the key 'TargetRegions' must be of the type 'list'."
                                        " The current type is %s."
                                        % type(test_region_key)
                                    )
                                    ...
                                    for region in range(0, len(test_region_key)):
                                        if not isinstance(
                                            test_region_key[region], dict
                                        ):
                                            opencortex.print_comment_v(
                                                "TypeError in input parameters: the list values of the key 'TargetRegions' must be of the type 'dict'."
                                                " The current type is %s."
                                                % type(test_region_key[region])
                                            )
                                            ...
                                            for dim_key in [
                                                ...
                                                if (
                                                    dim_key
                                                    not in test_region_key[
                                                        region
                                                    ].keys()
                                                ):
                                                    opencortex.print_comment_v(
                                                        "ValueError in input parameters: the list values of the key 'TargetRegions' must be dictionaries "
                                                    ...
                                                    if not isinstance(
                                                        test_region_key[region][
                                                            dim_key
                                                        ],
                                                        list,
                                                    ):
                                                        opencortex.print_comment_v(
                                                            "TypeError in input parameters: the 'X/Y/ZVector' must store the value of type 'list'."
                                                            " The current type is %s."
                                                            % type(
                                                                test_region_key[region][
                                                                    dim_key
                                                        ...
                                                        if (
                                                            ...
                                                            opencortex.print_comment_v(
                                                                "ValueError in input parameters: the lists stored by 'XVector', 'YVector' and 'ZVector'"
                                ...
                                opencortex.print_comment_v(
                                    "KeyError in input parameters: 'LocationSpecific' is True but the key 'TargetRegions' is not in input parameters."
                                )
                    ...
                    opencortex.print_comment_v(
                        "KeyError in input parameters: the key 'LocationSpecific' is not in input parameters."
                    )
