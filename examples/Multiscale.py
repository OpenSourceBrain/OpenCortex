...
def generate(
    ...
    popExc2 = oc.add_population_in_rectangular_region(
        network,
        "popExc2",
        "L23_NoHotSpot",
        num_exc2,
        ...
        for pop1 in allExc:
            for pop2 in allExc:
                proj = oc.add_probabilistic_projection(
                    network, "proj0", pop1, pop2, synAmpa1.id, 0.5, delay=global_delay
                )

            proj = oc.add_probabilistic_projection(
                network, "proj1", pop1, popInh, synAmpa1.id, 0.7, delay=global_delay
            )

            proj = oc.add_probabilistic_projection(
                network, "proj2", popInh, pop1, synGaba1.id, 0.7, delay=global_delay
            )

        proj = oc.add_probabilistic_projection(
            network, "proj3", popInh, popInh, synGaba1.id, 0.5, delay=global_delay
        )
    ...
    for pop in allExc:
        oc.add_inputs_to_population(
            network, "Stim_%s" % pop.id, pop, pfs1.id, all_cells=True
        )
    ...
    if format == "xml":
        ...
        for i in range(min(max_in_pop_to_plot_and_save, num_exc2)):
            ...
            save_v[exc2_traces].append(
                "%s/%i/%s/v" % (popExc2.id, i, popExc2.component)
            )
        ...
        if run_in_simulator:
            ...
            traces, events = oc.simulate_network(
                lems_file_name,
                run_in_simulator,
                max_memory="4000M",
                nogui=True,
            ...
            print(
                "Run %s: Exc rate: %s Hz; Inh rate %s Hz"
...
def _plot_(X, g_rng, i_rng, sbplt=111, ttl=[]):
    ...
    pl.ylabel("Input (Hz)")
    ax.set_xticks(range(0, len(g_rng)))
    ax.set_xticklabels(g_rng)
    ax.set_yticks(range(0, len(i_rng)))
    ax.set_yticklabels(i_rng)
    pl.colorbar()
...
if __name__ == "__main__":
    if "-all" in sys.argv:
        ...
        generate(scalePops=5, scalex=2, scalez=2, connections=False)
            ...
            for i2, i in enumerate(i_rng):
                ...
                for h in trace_highlight:
                    if h[0] == g and h[1] == i:
                        highlight = True
                print(
                    " Run %s of %s: g = %s; i=%s (highlighting: %s)"
                    % (count, len(g_rng) * len(i_rng), g, i, highlight)
                )
                info = generate(
                    scalePops=scalePops,
                    scalex=2,
        ...
        pl.savefig(
            "%s_%s_%sms.png" % (run_in_simulator, scalePops, duration),
            bbox_inches="tight",
        )
        print("Finished: " + info)
        pl.show()
