...
def generate(
    ...
    if num_bbp > 0:
        oc.include_opencortex_cell(
            nml_doc, "BlueBrainProject_NMC/cADpyr229_L23_PC_5ecbf9b163_0_0.cell.nml"
        )
    ...
    pfs1 = oc.add_poisson_firing_synapse(
        nml_doc, id="psf1", average_rate="%s Hz" % input_rate, synapse_id=synAmpa1.id
    )

    #####   Populations

    popExc = oc.add_population_in_rectangular_region(
        network,
        "popExc",
        "HH_477127614",
        num_exc,
    ...
    popInh = oc.add_population_in_rectangular_region(
        network,
        "popInh",
        "HH_476686112",
        num_inh,
    ...
    if num_bbp == 1:
        popBBP = oc.add_single_cell_population(
            network, "popBBP", "cADpyr229_L23_PC_5ecbf9b163_0_0", z=200, color="0 .8 0"
        )
    elif num_bbp > 1:
    ...
    total_conns = 0
    if connections:
        proj = oc.add_probabilistic_projection(
            network, "proj0", popExc, popExc, synAmpa1.id, 0.5, delay=global_delay
        )
        total_conns += len(proj.connection_wds)

        proj = oc.add_probabilistic_projection(
            network, "proj1", popExc, popInh, synAmpa1.id, 0.7, delay=global_delay
        )
        total_conns += len(proj.connection_wds)

        proj = oc.add_probabilistic_projection(
            network, "proj3", popInh, popExc, synGaba1.id, 0.7, delay=global_delay
        )
        total_conns += len(proj.connection_wds)

        proj = oc.add_probabilistic_projection(
            network, "proj4", popInh, popInh, synGaba1.id, 0.5, delay=global_delay
        )
        total_conns += len(proj.connection_wds)

        if num_bbp > 0:
            proj = oc.add_probabilistic_projection(
                network, "proj5", popExc, popBBP, synAmpa1.id, 0.5, delay=global_delay
            )
    ...
    oc.add_inputs_to_population(network, "Stim0", popExc, pfs1.id, all_cells=True)

    #####   Save NeuroML and LEMS Simulation files

    if num_bbp != 1:
        new_reference = "Balanced_%scells_%sconns" % (
            num_bbp + num_exc + num_inh,
            total_conns,
        )
    ...
    oc.save_network(nml_doc, nml_file_name, validate=(format == "xml"), format=format)
...
if __name__ == "__main__":
    if "-all" in sys.argv:
        ...
        generate(num_bbp=10, scalePops=5, scalex=2, scalez=2, connections=False)

        generate(num_bbp=10, scalePops=5, scalex=2, scalez=2, global_delay=2)

        generate(
            num_bbp=0, scalePops=2, scalex=2, scalez=2, duration=2000, global_delay=2
        )
        ...
        generate(gen_spike_saves_for_all_somas=True, global_delay=2)
