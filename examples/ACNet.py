...
def generate(
    ...
    pop_pyr = oc.add_population_in_rectangular_region(
        network,
        "pop_pyr",
        "pyr_4_sym",
        num_pyr,
    ...
    ampa_syn_inh = oc.add_exp_two_syn(
        nml_doc,
        id="AMPA_syn_inh",
        gbase="0.15e-9S",
        erev="0mV",
    ...
    gaba_syn_inh = oc.add_exp_two_syn(
        nml_doc,
        id="GABA_syn_inh",
        gbase="0S",
        erev="-0.080V",
    ...
    pfs = oc.add_poisson_firing_synapse(
        nml_doc, id="poissonFiringSyn", average_rate="30 Hz", synapse_id=ampa_syn.id
    )

    total_conns = 0
    if connections:
        this_syn = ampa_syn.id
        proj = oc.add_targeted_projection(
            network,
            "Proj_pyr_pyr",
            pop_pyr,
            pop_pyr,
    ...
    lems_file_name = oc.generate_lems_simulation(
        ...
        target_dir=target_dir,
        report_file_name="report.acnet.txt",
    )
...
if __name__ == "__main__":
    if "-test" in sys.argv:
        generate(num_pyr=2, num_bask=2, duration=500, global_delay=2)
