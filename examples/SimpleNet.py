...
def generate(reference="SimpleNet", scale=1, format="xml"):
    ...
    pop = oc.add_population_in_rectangular_region(
        network, "RS_pop", "RS", population_size, 0, 0, 0, 100, 100, 100, color="0 .8 0"
    )
    import neuroml

    pop.properties.append(neuroml.Property("radius", 10))

    syn = oc.add_exp_two_syn(
        nml_doc, id="syn0", gbase="2nS", erev="0mV", tau_rise="0.5ms", tau_decay="10ms"
    )

    pfs = oc.add_poisson_firing_synapse(
        nml_doc, id="poissonFiringSyn", average_rate="50 Hz", synapse_id=syn.id
    )
