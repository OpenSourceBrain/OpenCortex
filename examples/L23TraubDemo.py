...
def generate(
    ...
    pop_rs = oc.add_population_in_rectangular_region(
        network, "pop_rs", "L23PyrRS", num_rs, 0, 0, 0, xDim, yDim, zDim, color=".8 0 0"
    )

    pop_bask = oc.add_population_in_rectangular_region(
        network,
        "pop_bask",
        "SupBasket",
        num_bask,
    ...
    syn0 = oc.add_exp_two_syn(
        nml_doc, id="syn0", gbase="1nS", erev="0mV", tau_rise="0.5ms", tau_decay="10ms"
    )

    syn1 = oc.add_exp_two_syn(
        nml_doc, id="syn1", gbase="2nS", erev="0mV", tau_rise="1ms", tau_decay="15ms"
    )
    ...
    if poisson_inputs:
        ...
        oc.add_inputs_to_population(network, "Stim0", pop_rs, pfs.id, all_cells=True)
        """                            
    ...
    if offset_curent_range_pA:
        for pop_id in offset_curent_range_pA:
            ...
            input_list = neuroml.InputList(
                id="inputs_offset_current_%s" % pop.id,
                component=pg0.id,
                populations=pop.id,
            )
    ...
    if num_rs != DEFAULT_RS_POP_SIZE or num_bask != DEFAULT_BASK_POP_SIZE:
        new_reference = "%s_%scells_%sconns" % (
            nml_doc.id,
            num_rs + num_bask,
            total_conns,
        )
            ...
            if pop_nml is not None and pop_nml.size > 0:
                for i in range(int(pop_nml.size)):
                    ...
                    gen_saves_for_quantities[
                        "Sim_%s.%s.%i.v.dat" % (nml_doc.id, pop, i)
                    ] = []

                    for seg in segments_to_plot_record[pop]:
                        quantity = "%s/%i/%s/%i/v" % (pop, i, pop_nml.component, seg)
                        gen_plots_for_quantities["Display_%s_%i_v" % (pop, i)].append(
                            quantity
                        )
                        gen_saves_for_quantities[
                            "Sim_%s.%s.%i.v.dat" % (nml_doc.id, pop, i)
...
if __name__ == "__main__":
    if "-test" in sys.argv:
        generate(num_rs=2, num_bask=0, duration=50, global_delay=2)
        ...
        generate(num_rs=8, num_bask=4, duration=200, connections=True, global_delay=2)
