...
def generate(
    reference="Weights", num_each=6, connections=True, duration=1000, format="xml"
):
    nml_doc, network = oc.generate_network(reference)

    cell_id = "HH_477127614"
    cell = oc.include_opencortex_cell(
        nml_doc, "AllenInstituteCellTypesDB_HH/%s.cell.nml" % cell_id
    )
    ...
    pop_pre = oc.add_population_in_rectangular_region(
        network, "pop_pre", cell_id, num_each, 0, 0, 0, xDim, yDim, zDim, color=".8 0 0"
    )

    pop_post_chem_exc = oc.add_population_in_rectangular_region(
        network,
        "pop_post_chem_exc",
        cell_id,
        num_each + 1,
    ...
    pop_post_chem_inh = oc.add_population_in_rectangular_region(
        network,
        "pop_post_chem_inh",
        cell_id,
        num_each + 2,
    ...
    pop_post_cont = oc.add_population_in_rectangular_region(
        network,
        "pop_post_cont",
        cell_id,
        num_each + 3,
    ...
    gj_syn = oc.add_gap_junction_synapse(nml_doc, id="gj0", conductance=".05nS")

    analog_syn = GradedSynapse(
        id="analog_syn",
        conductance="10nS",
        delta="5mV",
        Vth="-35mV",
    ...
    pfs = oc.add_poisson_firing_synapse(
        nml_doc, id="poissonFiringSyn", average_rate="10 Hz", synapse_id=ampa_syn.id
    )

    oc.add_inputs_to_population(network, "Stim0", pop_pre, pfs.id, all_cells=True)

    if connections:
        proj_chem_exc = oc.add_probabilistic_projection(
            network,
            "proj_chem_exc",
            pop_pre,
            pop_post_chem_exc,
        ...
        proj_chem_inh = oc.add_probabilistic_projection(
            network,
            "proj_chem_inh",
            pop_pre,
            pop_post_chem_inh,
        ...
        proj_cont = ContinuousProjection(
            id="proj_cont",
        ...
        for pre in gj_pops:
            for post in gj_pops:
                proj_gap = oc.add_targeted_electrical_projection(
                    nml_doc,
                    network,
                    "proj_",
                    pre,
                ...
                for conn in network.electrical_projections[
                    -1
...
if __name__ == "__main__":
    if "-test" in sys.argv:
        generate(num_pre=1, duration=1000)
